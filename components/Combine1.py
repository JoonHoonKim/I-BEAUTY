...
def Enlargement():
    ...
    while True:
        ...
        if faces is not ():
            print('face detected, press \'q\' to quit Enlargement')
            while True:
                ret, frame = cap.read()
                ...
                cv2.line(resizedroi, (int(width/3), 0), (int(width/3), height), red_color)
                cv2.line(resizedroi, (int(width*2/3), 0), (int(width*2/3), height), red_color)
                cv2.line(resizedroi, (0, int(height/3)), (width, int(height/3)), red_color)
                cv2.line(resizedroi, (0, int(height*2/3)), (width, int(height*2/3)), red_color)
...
def control():
    while True:
        key = input('press \'e\' to start Enlargement')
        if key == 'e':
